...
import { 
  ...
  HeartPulse, Zap, Home, Menu, Phone, Sparkles, PlusCircle, CreditCard, Calendar, Pill,
  UserPlus, TrendingUp, Eye, Receipt
} from 'lucide-react';
import { UserRole, AnalysisResult, PrescriptionData } from './types';
...
const App: React.FC = () => {
  ...
  const [currentView, setCurrentView] = useState<'HOME' | 'PRESCRIPTION' | 'SCHEDULE' | 'DRUG_INFO' | 'GROWTH' | 'BILLING'>('HOME');
      ...
      <div className="w-full max-w-md bg-white shadow-2xl md:rounded-[2.5rem] overflow-hidden border border-gray-200 min-h-screen md:min-h-[850px] flex flex-col relative">
        ...
        <header className="bg-white p-4 flex justify-between items-center sticky top-0 z-40 border-b border-gray-100">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative"><Bell className="w-5 h-5 text-slate-500" /><span className="absolute -top-1 -right-1 w-2 h-2 bg-red-500 rounded-full border-2 border-white"></span></div>
        ...
        <main className="flex-1 overflow-y-auto pb-32 scrollbar-hide">
          {role === UserRole.DOCTOR ? (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              <section className="bg-slate-50/80 py-6 border-b border-slate-100">
                <div className="px-5 mb-4 flex justify-between items-end">
                  ...
                  <span className="text-blue-600 text-xs font-bold bg-blue-50 px-3 py-1 rounded-full border border-blue-100">Practice Overview</span>
                </div>
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              <section className="py-6"><div className="px-5"><h2 className="text-slate-800 font-extrabold text-2xl mb-5 tracking-tight">Patient Zone</h2><form onSubmit={handleSearch} className="relative mb-6 group"><input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Ask AI about symptoms..." className="w-full pl-12 pr-12 py-4 rounded-[1.25rem] bg-slate-50 border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none transition-all shadow-sm text-sm" /><Search className="absolute left-4 top-4 text-slate-400 group-focus-within:text-blue-500 w-5 h-5" /><button type="submit" className="absolute right-3 top-2.5 p-1.5 bg-blue-600 rounded-xl text-white shadow-lg shadow-blue-200 hover:bg-blue-700 transition-colors"><Sparkles size={18} /></button></form><div className="grid grid-cols-4 gap-4 mb-8">{[{ icon: <User className="text-blue-600"/>, label: "Doctors" }, { icon: <Home className="text-teal-600"/>, label: "Clinics" }, { icon: <FileText className="text-indigo-600"/>, label: "Hospital" }, { icon: <Activity className="text-pink-600"/>, label: "Day Care" }].map((item, i) => (<div key={i} className="flex flex-col items-center gap-2 cursor-pointer group"><div className="w-14 h-14 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center group-hover:bg-white group-hover:shadow-md group-hover:-translate-y-1 transition-all">{item.icon}</div><span className="text-[10px] font-bold text-slate-700 text-center leading-tight">{item.label}</span></div>))}</div></div></section>
        ...
        <nav className="absolute bottom-0 w-full bg-white/80 backdrop-blur-md border-t border-slate-100 py-4 px-8 flex justify-between items-center z-40">
          ...
          <div className={`flex flex-col items-center gap-1 cursor-pointer text-slate-400 hover:text-slate-600 transition-colors`}><Menu size={26}/><span className="text-[9px] font-bold uppercase tracking-widest">Menu</span></div>
        </nav>
